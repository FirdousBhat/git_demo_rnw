...
class Products {
    ...
    function process() {
        ...
        $allProducts = $this->getRelatedProducts($cat_id,$pid);
    ...
    function getRelatedProducts($cat_id,$pid) {
        include("config.php");
        if (!$page) {
            $page = 0;
        }
       
        $sql = "SELECT * FROM gf_products where cat_id=$cat_id AND id NOT IN ($pid) group by rel_priority";
